...
/**
 * =============================================================================
 * bootstrap.php — Arranque e inicialización del framework osTicket
 * =============================================================================
 *
 * Propósito:
 *   Archivo central de bootstrap de osTicket. Es el primero en cargarse en
 *   cualquier punto de entrada (web, CLI, API). Define constantes globales de
 *   rutas, versión y prefijo de tablas; configura el entorno PHP; conecta con
 *   la base de datos y carga las clases esenciales del sistema.
 *
 * La clase Bootstrap expone métodos estáticos que se invocan en orden:
 *   1. Bootstrap::init()          → Seguridad, ini_set, timezone, polyfills
 *   2. Bootstrap::loadConfig()    → Localiza y carga ost-config.php
 *   3. Bootstrap::defineTables()  → Define constantes con nombres de tablas
 *   4. Bootstrap::connect()       → Conecta a MySQL (soporta múltiples hosts)
 *   5. Bootstrap::loadCode()      → require_once de todas las clases core
 *   6. Bootstrap::i18n_prep()     → UTF-8, mbstring polyfills, random_int
 *
 * Constantes de ruta definidas al final del archivo (fuera de la clase):
 *   ROOT_DIR      → Directorio raíz de la instalación (path real del SO)
 *   INCLUDE_DIR   → ROOT_DIR/include/
 *   PEAR_DIR      → INCLUDE_DIR/pear/
 *   CLIENTINC_DIR → INCLUDE_DIR/client/
 *   STAFFINC_DIR  → INCLUDE_DIR/staff/
 *   I18N_DIR      → INCLUDE_DIR/i18n/
 *   ... (ver sección de constantes al pie)
 *
 * Constantes de versión:
 *   THIS_VERSION  → Versión de osTicket activa (ej: v1.18.3)
 *   MAJOR_VERSION → Versión mayor (ej: 1.18)
 *   GIT_VERSION   → Hash corto del commit (ej: 2570d69)
 *
 * Dependencias:
 *   - PHP 5.6+ (polyfills para mbstring y random_int incluidos)
 *   - include/class.osticket.php, class.misc.php, class.http.php
 *   - include/ost-config.php (credenciales de BD, prefijo de tablas)
 *
 * Seguridad:
 *   - Desactiva register_globals, allow_url_fopen, allow_url_include.
 *   - Desactiva session IDs en URL (session.use_trans_sid = 0).
 *   - No muestra errores en pantalla (display_errors = 0).
 *
 * @package    osTicket
