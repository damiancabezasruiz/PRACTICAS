...
/**
 * =============================================================================
 * incidencias.php — Gestión principal de incidencias (CRUD + adjuntos)
 * =============================================================================
 *
 * Propósito:
 ...
 *     - ost_form_entry      → entrada de formulario personalizado (form_id=18)
 ...
 *   - osTicket instalado en ROOT_DIR con form_id=18 configurado
 *   - Extensión mysqli
 *   - Constante FORM_INCIDENCIAS_ID = 18
 *
...
<head>
    ...
    <title>Acceso — Incidencias ATU</title>
    <style>
...
<div class="login-card">
    ...
    <div class="login-title">Área restringida</div>
    <div class="login-sub">Gestión de Incidencias · ATU</div>

    <?php if (!empty($login_error)): ?>
        <div class="error-msg">⚠️ <?= htmlspecialchars($login_error) ?></div>
    <?php endif; ?>

    <form method="POST" action="incidencias.php" autocomplete="off">
        <div class="field">
            <input type="text" id="usr" name="_login_user"
                   placeholder="Usuario"
                   autofocus required autocomplete="username"
                   value="<?= htmlspecialchars($_POST['_login_user'] ?? '') ?>">
        </div>
        <div class="field">
            <input type="password" id="pwd" name="_login_password"
                   placeholder="Contraseña"
                   required autocomplete="current-password">
            <button type="button" class="eye" onclick="togglePwd()" title="Mostrar/ocultar">👁</button>
        </div>
        <button type="submit" class="btn-login">Entrar</button>
    </form>

    <div class="footer-note">Acceso exclusivo para personal autorizado</div>
</div>
<script>
    function togglePwd() {
...
function sync_form_to_cdata(mysqli $db, int $ticket_id, int $form_id = 18): void {
    ...
    // 🔹 SOLO columnas válidas (evita error 500)
    $columnas_validas = [
...
sync_form_to_cdata($conexion, $nuevoTid, 18);
...
// Definir campos a seleccionar en la consulta
$campos = [
    'subject','plan','sector',
    'sector_labora','sector_cyl','sector_asturias','sector_estatal',
    'tutor','nombreAlu','apellidosAlu','empresa','accion','grupo','curso',
    'incidencia','detalles','razon','dificultad','datos','motivo','Archivo',
    'fotos','medidas','solucion_manual','fecha_solucion_manual','estado_texto','priority'
];

// Construir SELECT dinámico buscando en formulario personalizado primero
$selects = [];
foreach ($campos as $c) {
    $selects[] = "COALESCE(
        (SELECT fev.value 
...
$filtros_individuales = [
    'subject' => get_array_param('subject'),
    'plan' => get_array_param('plan'),
    'sector' => get_array_param('sector'),
    'tutor' => get_array_param('tutor'),
    'nombreAlu' => get_array_param('nombreAlu'),
    'apellidosAlu' => get_array_param('apellidosAlu'),
    'empresa' => get_array_param('empresa'),
    'accion' => get_array_param('accion'),
    'grupo' => get_array_param('grupo'),
    'curso' => get_array_param('curso'),
    'incidencia' => get_array_param('incidencia'),
    'detalles' => get_array_param('detalles'),
    'razon' => get_array_param('razon'),
    'dificultad' => get_array_param('dificultad'),
    'datos' => get_array_param('datos'),
...
LEFT JOIN ost_ticket__cdata cd ON cd.ticket_id=t.ticket_id
WHERE t.topic_id NOT IN (13)";

$sql_where = " AND COALESCE(cd.subject,'') NOT LIKE '%Valoración%' AND t.topic_id != 13";
...
if ($f_buscar !== '') {
    ...
    $sql_where .= " AND (
        COALESCE(cd.subject,'') LIKE '%{$b}%'
        OR COALESCE(cd.plan,'') LIKE '%{$b}%'
        OR COALESCE(cd.sector,'') LIKE '%{$b}%'
        OR COALESCE(cd.nombreAlu,'') LIKE '%{$b}%'
        OR COALESCE(cd.apellidosAlu,'') LIKE '%{$b}%'
        OR COALESCE(cd.empresa,'') LIKE '%{$b}%'
        OR COALESCE(cd.tutor,'') LIKE '%{$b}%'
        OR COALESCE(cd.accion,'') LIKE '%{$b}%'
        OR COALESCE(cd.grupo,'') LIKE '%{$b}%'
        OR COALESCE(cd.curso,'') LIKE '%{$b}%'
        OR COALESCE(cd.incidencia,'') LIKE '%{$b}%'
        OR COALESCE(cd.motivo,'') LIKE '%{$b}%'
        OR COALESCE(cd.detalles,'') LIKE '%{$b}%'
        OR COALESCE(cd.razon,'') LIKE '%{$b}%'
        OR COALESCE(cd.dificultad,'') LIKE '%{$b}%'
        OR COALESCE(cd.datos,'') LIKE '%{$b}%'
        OR COALESCE(cd.sector_labora,'') LIKE '%{$b}%'
        OR COALESCE(cd.sector_cyl,'') LIKE '%{$b}%'
        OR COALESCE(cd.sector_asturias,'') LIKE '%{$b}%'
        OR COALESCE(cd.sector_estatal,'') LIKE '%{$b}%'
        OR t.number LIKE '%{$b}%'
    )";
}
...
// Aplicar filtros individuales por campo
foreach ($filtros_individuales as $campo => $valor) {
    ...
    foreach ($valor as $v) {
        ...
        if ($campo === 'sector') {
    // Si hay filtro de plan, solo mostrar sectores válidos para ese plan
    if (!empty($filtros_individuales['plan'])) {
        $plan_val = reset($filtros_individuales['plan']); // Tomar el primer plan
        $plan_norm = mb_strtolower(normalizar_texto($plan_val), 'UTF-8');
        
        // Detectar qué columnas de sector corresponden a este plan
        $sector_col_valida = detectar_columna_sector_por_plan($plan_norm);
        
        if ($sector_col_valida !== '') {
            // Solo buscar en la columna del sector correspondiente
            $condiciones[] = "COALESCE(cd.`{$sector_col_valida}`,'') = '{$valorEsc}'";
        } else {
            // Plan sin sector específico (como forcacem)
            $condiciones[] = "COALESCE(cd.`sector`,'') = '{$valorEsc}'";
        }
    } else {
        // Sin filtro de plan, buscar en todas las columnas
        $sectorCols = ['sector','sector_labora','sector_cyl','sector_asturias','sector_estatal'];
        $sub = [];
        foreach ($sectorCols as $sc) {
            $sub[] = "COALESCE(cd.`$sc`,'') = '{$valorEsc}'";
        }
        $condiciones[] = "(" . implode(' OR ', $sub) . ")";
    }
} elseif ($campo === 'subject') {
    // "subject" es en realidad el expediente en el formulario
    $condiciones[] = "COALESCE(cd.`subject`,'') = '{$valorEsc}'";
} else {
    $condiciones[] = "COALESCE(cd.`{$campo}`,'') LIKE '%{$valorEsc}%'";
}
    }

    // IMPORTANTE: ahora cada campo es independiente
    $sql_where .= " AND (" . implode(' OR ', $condiciones) . ")";
}

// Filtro especial para motivo (búsqueda por LIKE)
if ($filtros_individuales['motivo'] !== '') {
    $motivoEsc = $conexion->real_escape_string($filtros_individuales['motivo']);
    $sql_where .= " AND COALESCE(cd.`motivo`,'') LIKE '%{$motivoEsc}%'";
}
...
// Validación: Si se aplicaron filtros, los contadores deben reflejar solo los tickets visibles
if ($total_resultados > 0 && array_sum($contadores) === 0) {
    ...
    try {
        $sq = $conexion->query("
            ...
            FROM ost_ticket t
            WHERE t.topic_id IN (1,12)" . $sql_where . "
            GROUP BY t.status_id
        ");
...
if (isset($_GET['excel'])) {
    ...
    header('Content-Disposition: attachment; filename="Incidencias_' . date('d-m-Y') . '.csv"');
    ...
    fputcsv($out, [
        ...
        'TUTOR/A','NOMBRE ALUMNO/A','APELLIDO ALUMNO/A',
        'EMPRESA','ACCIÓN','GRUPO','CURSO','INCIDENCIA',
        'DETALLES','RAZÓN','DIFICULTAD','DATOS',
        'MOTIVOS','FOTOS','MEDIDAS TOMADAS','SOLUCIÓN','FECHA SOLUCIÓN',
        'ESTADO','TICKET'
    ], ',');
...
========================================================= */
$opciones_filtros = [
    'subject' => obtener_opciones_filtro($conexion, 'subject'),
    'plan' => obtener_opciones_filtro($conexion, 'plan'),
    'sector' => obtener_opciones_sector_unificado($conexion),
    'tutor' => obtener_opciones_filtro($conexion, 'tutor'),
    'nombreAlu' => obtener_opciones_filtro($conexion, 'nombreAlu'),
    'apellidosAlu' => obtener_opciones_filtro($conexion, 'apellidosAlu'),
    'empresa' => obtener_opciones_filtro($conexion, 'empresa'),
    'accion' => obtener_opciones_filtro($conexion, 'accion'),
    'grupo' => obtener_opciones_filtro($conexion, 'grupo'),
    'curso' => obtener_opciones_filtro($conexion, 'curso'),
    'incidencia' => obtener_opciones_filtro($conexion, 'incidencia'),
    'detalles' => obtener_opciones_filtro($conexion, 'detalles'),
    'razon' => obtener_opciones_filtro($conexion, 'razon'),
    'dificultad' => obtener_opciones_filtro($conexion, 'dificultad'),
    'datos' => obtener_opciones_filtro($conexion, 'datos')
];
...
<title>Incidencias - Grupo ATU</title>
...
/* ==========================================================
   LÓGICA PARA OCULTAR HEADER (VERSIÓN DE DIAGNÓSTICO)
========================================================== */

// --- INICIO DEL CÓDIGO DE SCROLL ---

let ultimaPosicionScroll = 0;

// Usamos el evento 'DOMContentLoaded' para asegurarnos de que todo el HTML está cargado
// antes de intentar seleccionar el header.
document.addEventListener("DOMContentLoaded", function() {
    
    const miHeader = document.querySelector('.header');

    // Si no encontramos el header, lo decimos en la consola y paramos.
    if (!miHeader) {
        console.error("Error Crítico: No se encontró el elemento con la clase '.header'.");
        return;
    }

    // Creamos la función que se ejecutará en cada scroll
    function manejarScroll() {
        let posicionActualScroll = window.pageYOffset || document.documentElement.scrollTop;

        // Para depurar: muestra la posición actual en la consola.
        // console.log('Scroll en:', posicionActualScroll);

        if (posicionActualScroll > ultimaPosicionScroll && posicionActualScroll > 100) {
            // Hacia abajo
    ...
    // Añadimos el "oyente" al objeto window.
...
/* ============================================================
   SIDEBAR
============================================================ */
.sidebar{position:fixed;top:0;left:0;width:var(--sidebar-width);height:100vh;background:var(--bg-sidebar);z-index:1000;transition:.3s;display:flex;flex-direction:column}
...
.sidebar-brand{font-size:20px;font-weight:800;color:#fff}
.sidebar-brand .short{display:none}
.sidebar.collapsed .sidebar-brand .full{display:none}
.sidebar.collapsed .sidebar-brand .short{display:inline}
.sidebar-menu{flex:1;padding:20px 10px}
.menu-item{display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:10px;color:#94a3b8;text-decoration:none;font-size:14px;font-weight:500;margin-bottom:4px;white-space:nowrap;overflow:hidden}
...
.sidebar-collapse-btn{width:100%;padding:14px;background:transparent;border:none;border-top:1px solid rgba(255,255,255,.06);color:#94a3b8;cursor:pointer}

/* ============================================================
   CONTENIDO PRINCIPAL
============================================================ */
.main-content{margin-left:var(--sidebar-width);transition:.3s;min-height:100vh}
.sidebar.collapsed~.main-content{margin-left:var(--sidebar-collapsed)}
.header{background:rgba(255,255,255,.95);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);padding:0 28px;height:var(--header-height);display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100}
.page-content{padding:24px 28px}

/* ============================================================
   BOTONES
============================================================ */
.btn{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;border:none;text-decoration:none}
...
/* ============================================================
   PANELES
============================================================ */
.estado-filter-panel,.filters-panel,.table-section{background:#fff;border-radius:14px;border:1px solid var(--border);margin-bottom:20px;overflow:visible}
...
.estado-btn.iniciada{
    border-color:var(--iniciada);
    background:var(--iniciada);
    color:#fff;
}
.estado-btn.en_curso{
    border-color:var(--en-curso);
    background:var(--en-curso);
    color:#0f172a;
}
.estado-btn.cerrada{
    border-color:var(--cerrada);
    background:var(--cerrada);
    color:#0f172a;
}
.estado-btn.enviada{
    border-color:var(--enviada);
    background:var(--enviada);
    color:#0f172a;
}
.estado-btn.sin_asignar{
    border-color:var(--sin-asignar);
    background:var(--sin-asignar);
    color:#fff;
}
.estado-btn.active{
    transform:translateY(-1px);
...
.cell-select option[value=""] {
    color: #cbd5e1;
    font-style: italic;
}	
	
/* ============================================================
 SELECT DESPLEGABLES AUTO-AJUSTABLES
============================================================ */
.cell-select-auto {
    width: auto;
    min-width: 140px;
    ...
    border-radius: 6px;
    font-size: 11px;
    background: #fff url('data:image/svg+xml;utf8,<svg fill="%23475569" height="16" viewBox="0 0 16 16" width="16" xmlns="http://www.w3.org/2000/svg"><path d="M4.427 7.427l3.396 3.396a.25.25 0 00.354 0l3.396-3.396A.25.25 0 0011.396 7H4.604a.25.25 0 00-.177.427z"/></svg>') no-repeat right 8px center;
...
.cell-select-auto:hover {
    border-color: #94a3b8;
    background-color: #f8fafc;
}

.cell-select-auto:focus {
    outline: none;
    border-color: var(--primary);
    box-shadow: 0 0 0 3px rgba(99, 102, 241, 0.1);
}

.cell-select-auto option {
    padding: 6px 10px;
    font-size: 11px;
    line-height: 1.5;
}

.cell-select-auto option[value=""] {
    color: #cbd5e1;
    font-style: italic;
}

/* Ajustar ancho del select según contenido */
.cell-select-auto option {
...
============================================================ */
.filters-header{display:flex;align-items:center;justify-content:space-between;padding:14px 20px;cursor:pointer}
.filters-body{max-height:1400px;overflow:visible;transition:max-height .4s,padding .4s,opacity .3s;padding:0 20px 20px;opacity:1;position:relative;z-index:300}
.filters-body.collapsed{max-height:0;padding:0 20px;opacity:0;overflow:hidden}
.filters-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}
.filter-group{display:flex;flex-direction:column;gap:4px;position:relative;z-index:350}
.filter-group label{font-size:11px;font-weight:600;color:var(--text-secondary);text-transform:uppercase}
.filter-group input,.filter-group select{height:38px;border:1.5px solid var(--border);border-radius:6px;padding:0 12px;font-size:13px;background:#fff}
...
.table-header-left{display:flex;align-items:center;gap:12px;flex-wrap:wrap}
.legend-bar{display:flex;align-items:center;gap:18px;padding:10px 20px;background:#f8fafc;border-bottom:1px solid var(--border);font-size:12px;flex-wrap:wrap}
...
/* ============================================================
   FLECHAS DE SCROLL HORIZONTAL (fijas en pantalla)
============================================================ */
.scroll-arrow-fixed{position:fixed;z-index:900;width:40px;height:40px;border-radius:50%;background:var(--primary);color:#fff;border:2px solid rgba(255,255,255,.9);box-shadow:0 3px 12px rgba(99,102,241,.4);display:flex;align-items:center;justify-content:center;font-size:15px;cursor:pointer;opacity:0;pointer-events:none;transition:opacity .2s ease,transform .15s ease,box-shadow .15s ease;-webkit-user-select:none;user-select:none}
.scroll-arrow-fixed.visible{opacity:1;pointer-events:auto}
.scroll-arrow-fixed:hover{background:var(--primary-dark);transform:scale(1.12);box-shadow:0 5px 18px rgba(99,102,241,.55)}
.scroll-arrow-fixed:active{transform:scale(.92)}

/* ============================================================
   CONTENEDOR DE TABLA CON SCROLL HORIZONTAL
============================================================ */
.table-container{overflow-x:auto;cursor:grab}
table{width:100%;border-collapse:collapse;min-width:2860px}
thead th{padding:10px 8px;font-size:9px;font-weight:700;text-transform:uppercase;color:var(--text-secondary);background:#f8fafc;border-bottom:1px solid var(--border);text-align:left;white-space:nowrap;position:sticky;top:0;z-index:10}

/* ============================================================
   FILAS DE LA TABLA (colores según estado)
============================================================ */
tbody tr.row-iniciada{background:var(--iniciada-row)}
tbody tr.row-en-curso{background:var(--en-curso-row)}
tbody tr.row-cerrada{background:var(--cerrada-row)}
tbody tr.row-enviada{background:var(--enviada-row)}
tbody tr.row-sin-asignar{background:var(--sin-asignar-row)}
tbody td{padding:12px 8px;font-size:11px;vertical-align:top;min-height:56px;height:auto}

/* ============================================================
   INPUTS Y TEXTAREAS EDITABLES EN CELDAS
============================================================ */
.cell-input,.cell-select{width:100%;min-width:90px;padding:8px 6px;border:1px solid var(--border);border-radius:4px;font-size:11px;background:#fff;min-height:38px;line-height:1.4;word-wrap:break-word;overflow-wrap:break-word}
.cell-textarea{width:100%;min-width:120px;min-height:68px;padding:8px 6px;border:1px solid var(--border);border-radius:4px;font-size:11px;background:#fff;resize:vertical;line-height:1.5;word-wrap:break-word;overflow-wrap:break-word;white-space:pre-wrap}
.cell-textarea-50{
...
.cell-input{resize:none;white-space:pre-wrap;overflow:hidden}

/* ============================================================
   INDICADORES DE ESTADO DE GUARDADO (✓ ⟳ ✗)
============================================================ */
.si{margin-left:3px;font-size:10px}
.si .ok{color:#10b981;display:none}
.si .ld{color:#6366f1;display:none}
.si .er{color:#ef4444;display:none}

/* ============================================================
   ENLACES A TICKETS (negrita, sin subrayado)
============================================================ */
.ticket-link{color:#000000;text-decoration:none;font-weight:600}
...
.page-btn{min-width:34px;height:34px;border-radius:6px;border:1px solid var(--border);background:#fff;color:var(--text-secondary);display:flex;align-items:center;justify-content:center;text-decoration:none}
.page-btn.active{background:#6366f1;color:#fff}

/* ============================================================
   TOM SELECT (selectores múltiples)
============================================================ */
.ts-wrapper{font-size:13px;position:relative;z-index:500}
...
.ts-dropdown{border:1px solid var(--border)!important;border-radius:8px!important;overflow:hidden;z-index:99999!important;background:#fff!important}
...
============================================================ */
.action-btn-sm{background:#eef2ff;color:#4f46e5;border:1px solid #c7d2fe;padding:4px 8px;border-radius:4px;cursor:pointer;transition:.2s;font-size:13px}
.action-btn-sm:hover{background:#4f46e5;color:#fff}
...
.modal-content{background:#fff;width:100%;max-width:980px;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,.22);overflow:hidden;display:flex;flex-direction:column;max-height:92vh}
...
.close-btn{background:none;border:none;font-size:24px;cursor:pointer;color:var(--text-muted);padding:0;line-height:1}
.close-btn:hover{color:var(--iniciada)}
.modal-body{padding:24px;overflow:auto}
.modal-footer{padding:16px 24px;border-top:1px solid var(--border);display:flex;gap:12px;justify-content:flex-end;background:#f8fafc}

/* ============================================================
   TABLA DE PREVISUALIZACIÓN EN MODAL
============================================================ */
.preview-sheet-wrap{
    overflow:auto;
    border:1px solid var(--border);
...
.preview-sheet tbody tr:nth-child(even) td{
    background:#f8fafc;
}
...
<nav class="sidebar-menu">
    <!-- INCIDENCIAS -->
    <a href="incidencias.php" class="menu-item active"><i class="fas fa-exclamation-triangle"></i><span>Tabla de Incidencias</span></a>
    ...
    <button class="sidebar-collapse-btn" onclick="document.getElementById('sidebar').classList.toggle('collapsed')"><i class="fas fa-chevron-left"></i></button>
</aside>
...
<main class="main-content">
    ...
    <header class="header">
        ...
        <div style="display:flex;align-items:center;gap:18px;">
            <div><i class="far fa-calendar-alt"></i> <span id="clock"><?= date('d/m/Y H:i:s') ?></span></div>
    ...
    <div class="page-content">
        ...
        ============================================================ -->
        <div class="estado-filter-panel">
            <div class="panel-title"><i class="fas fa-filter"></i> Filtrar por Estado</div>
            <div class="estado-filter-row">
                <?php $bp = array_diff_key($params_pag, ['estado'=>'','pagina'=>'']); ?>
                <a href="?<?= http_build_query($bp+['estado'=>'todos']) ?>" class="estado-btn todos<?= $f_estado==='todos'?' active':'' ?>">Todos <span class="count"><?= $total_tickets ?></span></a>
                <a href="?<?= http_build_query($bp+['estado'=>'iniciada']) ?>" class="estado-btn iniciada<?= $f_estado==='iniciada'?' active':'' ?>">Iniciada <span class="count"><?= $contadores['iniciada'] ?></span></a>
                <a href="?<?= http_build_query($bp+['estado'=>'en_curso']) ?>" class="estado-btn en_curso<?= $f_estado==='en_curso'?' active':'' ?>">En Curso <span class="count"><?= $contadores['en_curso'] ?></span></a>
                <a href="?<?= http_build_query($bp+['estado'=>'cerrada']) ?>" class="estado-btn cerrada<?= $f_estado==='cerrada'?' active':'' ?>">Cerrada <span class="count"><?= $contadores['cerrada'] ?></span></a>
                <a href="?<?= http_build_query($bp+['estado'=>'enviada']) ?>" class="estado-btn enviada<?= $f_estado==='enviada'?' active':'' ?>">Enviada <span class="count"><?= $contadores['enviada'] ?></span></a>
                <a href="?<?= http_build_query($bp+['estado'=>'sin_asignar']) ?>" class="estado-btn sin_asignar<?= $f_estado==='sin_asignar'?' active':'' ?>">Sin Asignar <span class="count"><?= $contadores['sin_asignar'] ?></span></a>
            </div>
        </div>
            ...
            <div class="filters-body collapsed" id="fb">
                <form method="GET">
                    ...
                    <div class="filters-grid">
                        ...
                        <!-- Filtros desplegables múltiples -->
                        <?= render_select_filtro_multi('subject','Expediente',$filtros_individuales['subject'],$opciones_filtros['subject']) ?>
                        <?= render_select_filtro_multi('plan','Plan',$filtros_individuales['plan'],$opciones_filtros['plan']) ?>
                        <?= render_select_filtro_multi('sector','Sector',$filtros_individuales['sector'],$opciones_filtros['sector']) ?>
                        <?= render_select_filtro_multi('tutor','Tutor/a',$filtros_individuales['tutor'],$opciones_filtros['tutor']) ?>
                        <?= render_select_filtro_multi('nombreAlu','Nombre Alumno/a',$filtros_individuales['nombreAlu'],$opciones_filtros['nombreAlu']) ?>
                        <?= render_select_filtro_multi('apellidosAlu','Apellidos Alumno/a',$filtros_individuales['apellidosAlu'],$opciones_filtros['apellidosAlu']) ?>
                        <?= render_select_filtro_multi('empresa','Empresa',$filtros_individuales['empresa'],$opciones_filtros['empresa']) ?>
                        <?= render_select_filtro_multi('accion','Acción',$filtros_individuales['accion'],$opciones_filtros['accion']) ?>
                        <?= render_select_filtro_multi('grupo','Grupo',$filtros_individuales['grupo'],$opciones_filtros['grupo']) ?>
                        <?= render_select_filtro_multi('curso','Curso',$filtros_individuales['curso'],$opciones_filtros['curso']) ?>
                        <?= render_select_filtro_multi('incidencia','Incidencia',$filtros_individuales['incidencia'],$opciones_filtros['incidencia']) ?>
                        <?= render_select_filtro_multi('detalles','Detalles',$filtros_individuales['detalles'],$opciones_filtros['detalles']) ?>
                        <?= render_select_filtro_multi('razon','Razón',$filtros_individuales['razon'],$opciones_filtros['razon']) ?>
                        <?= render_select_filtro_multi('dificultad','Dificultad',$filtros_individuales['dificultad'],$opciones_filtros['dificultad']) ?>
                        <?= render_select_filtro_multi('datos','Datos',$filtros_individuales['datos'],$opciones_filtros['datos']) ?>
                        
                        <div class="filter-group"><label>Motivo</label><input type="text" name="motivo" value="<?= htmlspecialchars($filtros_individuales['motivo']) ?>" placeholder="Escribe el motivo"></div>
        ...
        <div class="table-section" id="tableSection">
            ...
            <div class="legend-bar">
                <span><span class="legend-dot iniciada"></span> Iniciada</span>
                <span><span class="legend-dot en-curso"></span> En Curso</span>
                <span><span class="legend-dot cerrada"></span> Cerrada</span>
                <span><span class="legend-dot enviada"></span> Enviada</span>
                <span><span class="legend-dot sin-asignar"></span> Sin Asignar</span>
            </div>
            ...
            <div class="table-container" id="tc">
                <table>
                    <thead>
                        <tr>
                            <th>Ver/Copiar - Duplicar - Eliminar</th>
                            <th>Fecha</th>
                            <th>Expediente</th>
                            <th>Plan</th>
                            <th>Sector Labora</th>
                            <th>Sector CYL</th>
                            <th>Sector Asturias</th>
                            <th>Sector Estatal</th>
                            <th>Tutor/a</th>
                    ...
                    <tbody id="tb">
                        <?php if ($resultado && $resultado->num_rows > 0): ?>
                            <?php while ($fila = $resultado->fetch_assoc()): ?>
                                <?= render_fila($fila, $mapa_estados) ?>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="27" style="text-align:center;padding:30px;color:#94a3b8">No se encontraron registros</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
function guardar(el,campo,tid){
    ...
    fetch('incidencias.php',{method:'POST',body:fd})
...
function recargarSectoresFila(planSelect) {
    ...
    fetch('incidencias.php?ajax_sectores_fila=1&plan=' + encodeURIComponent(plan))
    .then(function(r){ return r.json(); })
    .then(function(d) {
        if (!d.ok) {
