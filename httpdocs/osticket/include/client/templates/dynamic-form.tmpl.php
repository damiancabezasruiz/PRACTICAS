...
?>
    ...
    foreach ($form->getFields() as $field) {
        ...
        // --- FILTRO GESTOR PERSONALIZADO ---
        // Consultamos si el ID de este campo está marcado como oculto en nuestra tabla
        $sql_hidden = "SELECT 1 FROM custom_hidden_fields WHERE field_id = ".db_input($field->get('id'))." AND hidden = 1";
        $res_hidden = db_query($sql_hidden);
        
        if ($res_hidden && db_num_rows($res_hidden)) {
            continue; // Si está en la tabla de ocultos, saltamos a la siguiente iteración
        }
        // --- FIN FILTRO ---
